...
public class PostController {
    ...
    @PostMapping("/{postId}/{comment}/addCommentToComment")
    public ResponseEntity<PostWithCommentCountDto>addCommentToComment(@PathVariable("commentAuthor") User commentAuthor, HttpServletRequest request, @RequestBody PostCommentDto postCommentDto){
        User user = (User) request.getAttribute("user");
        Post post = postService.addCommentToComment(user,commentAuthor,postCommentDto);
        PostWithCommentCountDto postWithCommentCountDto = new PostWithCommentCountDto(post, post.getComments().size());
        return new ResponseEntity<>(postWithCommentCountDto, HttpStatus.CREATED);
    }
